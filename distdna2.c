	      ...
	      enum correction_t correction, int ambig, int oflags)
{
    ...
    for (i = 0; i < nseqs; i++) {
        if (oflags & PHYLIP) {
	    /* spit out PHYLIP 10-char truncated name */
	    char snam[11];
	    int count;
...
void
par_write_distmat(char *outfn, char **seqs, char **seqnames,
	      int nseqs, int seqlen, double gapwt, double a,
	      enum correction_t correction, int ambig, int oflags)
{
    int i, j;
    ...
    /* strictly lower or upper triangular */
    /*
     * A disgression on lower triangular matrices
     *
     *	A lower triangular matrix is a matrix where values i,j such that 
     *  j>i = 0:
     *
     *		1
     *		2 3
     ...
     *		. . . .
     *
     *	It can be represented compactly as an array where entry (i,j) is
     * 	stored at position ((i * (i-1)) / 2) + j
     *
     *	We can compute location of an element from the linear array in the
     *	matrix by noting that the row sizes increase by one each subsequent
     *	row, hence the index of the end of each row constitutes a triangular
     *	number series:
     *
     *		size of row 1 = 1	index of end of row 1 = 1
     *		size of row 2 = 2	index of end of row 2 = 1 + 2 = 3
     *		size of row 3 = 3	index of end of row 3 = 1 + 2 + 3 = 6
     *		. . .
     *		1, 3, 6, 10, 15, 21, 28...
     *
     *	Hence, elements of e.g. row 6 in the lower triangular matrix, must be 
     *	in between linear array values 15 and 21. This means that we can compute
     *	the row and column of any position in the linear array using integer
     *	arithmetic and the solution of a second degree equation:
     *
     *	As per the triangular number series, each row x ends in position
     *
     *		(n * (n + 1)) / 2 = row_end
     *
     *		(n^2 + n) / 2 = row_end
     ...
     *		                2
     *
     *	If instead of row_end we use the item position in the linear array,
     *  we will get a number in between the preceding triangular number (which
     *  corresponds to the end of the preceding row) and the subsequent 
     *  triangular number (which corresponds to the end of its row). The 
     *	real square root will not be exact, but using the integer square root
     * 	(i.e. rounded to zero with floor) or the integer solution of the
     *	equation will give us the index of the preceding row. The remainder
     *	can then be used to compute the row offset (column).
     *
     *	row = ((-1 * sqrt(8*pos)) / 2) + 1
     *
     *	col = ((-1 * sqrt(8*pos)) % 2) * row 	// row = row_size
     *
     *	NOTE: all this has been done using one-offset.
     *
     *	Now to integer square root: we can use the algorithm from
     ...
     *	    
     *	    for (int i = 0; i < 16; i++) {	/ * 16 for 32 bit numbers * /
     *	        root <<= 1;
     *		rem = (rem << 2) + (a >> 30);
     *		a <<= 2;
     *		root++;
     ...
     *	}
     *
     *	to get both the square root and the remainder.
     *
     *	or the codecodex.com wiki implementation:
     *
     *	unsigned long
     *	isqrt(x)
     *	unsigned long x;
    ...
    comparisons = nseqs * (nseqs - 1);
    dist = malloc(maxcpu * sizeof(double));
    for (i = 0; i < comparisons; i++) {
        /* the coordinates to compare are [i/nseqs]*[i%nseqs] */
        spawn(compute_distance, i%nseqs, i/nseqs, params, i/maxcpu);
	if (i/maxcpu == maxcpu -1)
	   output_dist
    }
    if (i/maxcpu != maxcpu-1)
    	output_dist:{
	   for j = 0; j<maxcpu; j++)
	     if (i/nseqs == 0) /* new line */
	         print(seqname[i%nseqs]);
	     print_dist(j)
