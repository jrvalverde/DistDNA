...
enum correction_t {
    ...
    CCV,		/** Corrected Composition Vector */
...
void write_distmat(char *outfn, char **seqs, char **seqnames,
		   int nseqs, int seqlen, double gapwt, double a, double ksize,
...
int main(int argc, char **argv)
{
    ...
    do {
	i = getopt_long(argc, argv, "i:o:g:a:?hl1pxsnk:", opts, &j);
	switch (i) {
	case '?':
	    usage(argv[0]);
	    break;
    ...
    write_distmat(outfn, seqs, seqnames, nseqs, seqlen, gapwt, a, ksize,
...
void usage(char *myname)
{
    ...
    puts("        --ccv          use corrected composition vectors");
    puts("                       (does not require aligned sequences or");
    puts("                       sequences with the same length)");
    puts("            -n         also consider ambiguity codes");
    puts("            -k #       k-mer size");
    puts("");
 ...
 */
		...
		case JUKES_CANTOR:
		    ...
		    break;
                case CCV:
                    dist = dist_ccv(seqs[i], seqs[j], ksize, ambig, 0);
                    break;
		case UNCORRECTED:
		default:
