...
int main(int argc, char *argv[])
{
    ...
    if (reference == NULL) {
	make_ccv(infile, outfile, ksize, ambig, correct);
    }
    else {
    	/* open sequence */
    	read_sequence(infile, &s, &slen);

    	/* compare two sequences (compute distance) */
	read_sequence(reference, &r, &rlen);
		
    	d = dist_ccv(s, r, ksize, ambig, correct);
	printf("Distance among sequences %s (%d) and %s (%d) is %1.9f\n", 
	        infile, slen, reference, rlen, d);
    }
    return 0;
}
#endif
...
void read_sequence(char *infile, char **seq, int *slen)
{
    ...
    if ((infile[0] == '-') && (infile[1] == '\0'))
      ...
      if ((in = fopen(infile, "r")) == NULL)
          ccv_error("Could not open input file");

    do {
      s = fgets(line, BUFSIZ, in);
      if (s == NULL) ccv_error("No  FASTA sequences in input file");
    } while ((line[0] != '>') && (line[0] != ';'));
    ...
    sl = 0; s[sl] = '\0';
    if (s == NULL) ccv_error("Not enough memory");
...
/**
 * only output sequence composition 
 */
void make_ccv(char *infile, char *outfile,int ksize, int ambig, int correct)
{
    int slen, rlen;
...
double dist_ccv(char *sequence, char *reference, int ksize, int ambig, int correct)
{
    char *s, *r;
    int slen, rlen;
    double *fcv, *fcv_1, *fcv_2, *s_ccv, *r_ccv, dist;
    ...
    s_ccv = calloc(sizeof(double), ipow(4, ksize));
    compute_corrected_CV(s_ccv, fcv, fcv_1, fcv_2, ksize); 
    ...
    /* compute corrected vector */
    r_ccv = calloc(sizeof(double), ipow(4, ksize));
    compute_corrected_CV(r_ccv, fcv, fcv_1, fcv_2, ksize); 

    /* we can release memory now */
    free(fcv); free(fcv_1); free(fcv_2);

    dist = compute_k_distance(s_ccv, r_ccv, ksize);
...
void print_cv(char *outfile, double *cv, int ksize)
{
    ...
    if ((outfile[0] == '-') && (outfile[1] == '\0'))
	    ...
	    ccv_error("Cannot open output file");
		 ...
		 double *f, double *f1, double *f2, int ksize)
{
	...
	for (j = 0; j < ksize; j++) {
		...
		break;
	    default:
		ccv_error("Sequence contains ambiguity codes");
